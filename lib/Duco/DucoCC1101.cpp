...
void DucoCC1101::initReceive()
{
	...
	*/	
	...
	writeBurstRegister(CC1101_PATABLE | CC1101_WRITE_BURST, (uint8_t*)PaTableReceive, 8);
...
void DucoCC1101::ducoToCC1101Packet(DucoPacket *, CC1101Packet *packet)
{
	packet->data[0] = 0;
	packet->data[1] = ->messageType;
	packet->data[2] = ->networkId[0];	
	packet->data[3] = ->networkId[1];	
	packet->data[4] = ->networkId[2];	
	packet->data[5] = ->networkId[3];	

	//address
	packet->data[6] = (->deviceIdSender << 3) | (->deviceIdReceiver >> 2);
	packet->data[7] = (->deviceIdReceiver << 6) | (->deviceIdSender << 1) | (->deviceIdReceiver2 >> 4);
	packet->data[8] = (->deviceIdReceiver2 << 4) | ->counter;

	// footer
	packet->data[9] = 0xFF; 

	for(uint8_t i=0; i < ->commandLength; i++){
		packet->data[10+i] = ->command[i];
	}

	for(uint8_t i=0; i < ->dataLength; i++){
		packet->data[10+->commandLength+i] = ->data[i];
	}

	packet->length = 10 + ->commandLength + ->dataLength;
}
